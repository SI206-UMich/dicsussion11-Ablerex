...
def create_patients_table(cur, conn):
    ...
    #ped id, name(string), species_id (number), age (integer), cuteness (integer), agressiveness (number)
...
def add_fluffle(cur, conn):
    ...
    #name = Fluffle, species = "Rabbit", age = 3, cuteness = 90, agressiveness = 100
    cur.execute("INSERT INTO Patients (id, name, species_id, age, cuteness, aggressiveness) VALUES(?,?,?,?,?,?)", ('0', 'Fluffle', 'O', '3', '90', '100'))
    conn.commit()
...
def add_pets_from_json(filename, cur, conn):
    ...
    for i in json_data:
        ...
        cur.execture('INSERT INTO Patients (pet_id, name, species_id, age, cuteness, aggressiveness) VALUES (?,?,?,?,?,?)', (pet_id, name, species, age, cuteness, aggressiveness))
        count += 1
    conn.commit()
